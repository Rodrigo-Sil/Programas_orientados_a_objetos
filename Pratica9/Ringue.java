public class Ringue {
    ...
    public String lutar() {
        ...
        if(lutadorUm.poderDeAtaque() > lutadorDois.poderDeAtaque()) {
            maiorAtaque = getLutadorUm().getNome();
            pontosL1++;
        ...
        if(pontosL1 > pontosL2){
            return String.format("Maior poder de ataque: %s\nMaior poder de defesa: %s\nO resultado e: %s", maiorAtaque, maiorDefesa, getLutadorUm().getNome());
        } else if(pontosL2 > pontosL1){
            return String.format("Maior poder de ataque: %s\nMaior poder de defesa: %s\nO resultado e: %s", maiorAtaque, maiorDefesa, getLutadorDois().getNome());
        } else{
